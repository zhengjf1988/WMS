...
<div class="container-fluid">
   ...
   <li class="active"><a href="Sctldetail/Sctldetail.viewls">历史记录</a></li>
</ul>
<div>&nbsp;</div>
					...
					<tbody>
						<c:forEach items="${sctldetailBeans}" var="sctl" varStatus="staturs">
							<tr>
								...
								<td><c:if test="${sctl.tlstatus==1}"><font color="red">进行中</font></c:if> <c:if test="${sctl.tlstatus==2}">已完成</c:if>  </td>
								<td><a onClick="openModal(${sctl.tlmxid})">详情</a></td>
           		...
           		$.ajax({
                    ...
                    success : function(data) {
                           ...
                           $.each(boardJSON, function(i, d) {
                                 ...
                                 ht=ht+"<div><label>发料信息</label><p><label>零件名称："+ljname+"</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label>库存数量："+d.kcl+"</label></p>"+
                           	    "<p><label>条码编号："+d.tmbh+"</label>&nbsp;&nbsp;&nbsp;&nbsp;<label>入库时间："+da.getFullYear()+"-"+(da.getMonth()+1)+"-"+da.getDate()+"</label></p><label>现在库位："+d.kwid+"</label></div>";
                                
                           
                           }); 
                            
                            ht=ht+"<br/><div><label>退料信息</label><p><label>零件名称："+ljname+"</label>&nbsp;&nbsp;;&nbsp;&nbsp;<label>退料数量："+sl+"</label></p></div>"
