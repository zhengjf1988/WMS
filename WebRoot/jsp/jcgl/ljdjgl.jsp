	...
	<div class="row" style="padding-left: 0">
		...
		<div class="col-md-10" style="overflow:auto;">
			...
			<hr>
				...
				<form action="Ljdj/Ljdjgl.view" class="form-horizontal" method="post">
				<div class="row">

					<div class="col-lg-2">
						
						<button type="button" class="btn btn-primary" data-toggle="modal"
							onclick="openModal()">添加零件等级</button>
					</div>
			...
			<div class="table-responsive" style="overflow:scroll;height: 380px;">
				<table class="table table-bordered" id="table">
					...
					<tbody>
								  ...
								  <c:forEach items="${ljdjglBeans}" var="ljdj" varStatus="staturs">
                       <tr>
                       <td>${staturs.index+1}</td>
                       <td>${ljdj.djname}</td>
                       <td>${ljdj.djmeno}</td>
                       <td><fmt:formatDate value="${ljdj.creDate}" pattern="yyyy-MM-dd" /></td>
                       <td>${ljdj.inputman}</td>
                       <td><a onclick="openupdate()">编辑</a>&nbsp;&nbsp;<a href="Ljdj/Ljdjgl.dele?djid=${ljdj.djid }">删除</a>&nbsp;&nbsp;<a onclick="openview()">详情</a></td>                   
                          
                       <td hidden="true">${ljdj.djid }</td>
                        </tr>
                       </c:forEach>
...
<form action="Ljdj/Ljdjgl.add" class="form-horizontal" role="form"
	method="post">
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="gridSystemModalLabel">零件等级</h4>
				</div>
				<div class="modal-body">
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="djname"
								name="djname">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-10">
							...
							<textarea rows="8" class="form-control" name="djmeno" id="djmeno"></textarea>
						</div>
					</div>
	...
	function openupdate(){
		$(document).on("click","#table tr:gt(0)",function(){
			  var rows = $(this).parent().find("tr");
				...
				$("#djname").val($(this).find("td:eq(1)").text());
				$("#djmeno").val($(this).find("td:eq(2)").text());
				$("#djid").val($(this).find("td:eq(6)").text());
